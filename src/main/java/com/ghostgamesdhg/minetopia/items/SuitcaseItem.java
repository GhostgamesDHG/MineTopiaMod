...
public class SuitcaseItem extends GmmModElements.ModElement {
	@ObjectHolder("gmm:suitcase")
	public static final Item block = null;
	public SuitcaseItem(GmmModElements instance) {
		super(instance, 54);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof SuitcaseGuiGuiWindow) {
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			super(new Item.Properties().group(MinetopiaExtra.TAB).maxStackSize(1).rarity(Rarity.COMMON));
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Suitcase");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new SuitcaseGuiGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
